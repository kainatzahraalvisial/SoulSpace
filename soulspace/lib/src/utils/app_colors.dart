import 'package:flutter/material.dart';

class AppColors {
  static const Color primary = Color(0xFF4A592A);
  static const Color secondary = Color(0xFF818C69);
  static const Color softCoral = Color(0xFFF2B2AC);
  static const Color warmPink = Color(0xFFF29B9B);
  static const Color background = Color(0xFFF2F2F2);
  static const Color offWhite = Color(0xFFF2F2F2);
  static const Color sage = Color(0xFF818C69);
  static const Color oliveGreen = Color(0xFF4A592A);
  static const Color black = Colors.black;
  static const Color lightBlack = Color(0xFF333333);
  static const Color white = Colors.white;
  static const Color forest = Color(0xFF4A592A);
  static const Color blush = Color(0xFFF2B2AC);
  static const Color peach = Color(0xFFFAD4BF);
  static const Color lightPink = Color(0xFFFFEAE9);
  static const Color gray = Color(0xFF888888);
  static const Color lightBackground = Color(0xFFFCEEEE); 
  static const Color lightpurple = Color(0xFFC177F2);
  static const Color yellow = Color(0xFFFDD214);




}
